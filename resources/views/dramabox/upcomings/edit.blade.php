        ...
        <div class="card p-4">
            ...
            <form action="{{ route('upcomings.update', $upcoming) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="3">{{ old('description', $upcoming->description) }}</textarea>
                    @error('description')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="category[]" id="category" class="form-select" multiple required>
                        @foreach($categories as $category)
                            <option value="{{ $category }}" {{ in_array($category, old('category', $upcoming->category ?? [])) ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    @if($upcoming->poster)
                        <div class="mt-2">
                            <img src="{{ $upcoming->poster_url }}" alt="Poster" width="100">
                ...
                <div class="mb-3">
                    ...
                    @if($upcoming->trailer)
                        <div class="mt-2">
                            <a href="{{ $upcoming->trailer_url }}" target="_blank">View Current Trailer</a>
